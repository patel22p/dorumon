...
public partial class VK : Base
{
    ...
    public enum Keys { playerstats = 1300 }
    ...
    public void Start(string url)
    {
        ...
        newThread(delegate()
        {
            try
            {
                ...
                Thread.Sleep(500);
                LocalUserV = GetUserInfo(userid);
                time = ToDate(int.Parse(GetGlobalVariable(0)));
                LastStatusTime = unixtime;
                _TimerA.AddMethod(delegate()
                {
                    new WWW2(LocalUserV.photo).done += delegate(WWW2 www)
                    {
                        print("loaded texture");
                        LocalUserV.texture = www.www.texture;
                        DontDestroyOnLoad(LocalUserV.texture);
                    };
                });
                int i = 500;
                ...
                _TimerA.AddMethod(onVkConnected);
            }
            catch (System.Exception e) { _Status = Status.disconnected; print("Login vkontakte failed, make sure you give the application all the rights" + e); }
        });
    }
    public Action onVkConnected;
    ...
    private void GetOwnStats(bool async)
    {
        newThread(async, delegate()
        {
            ...
            {
                LocalUserV.totalKills = int.Parse(ss[0]);
                LocalUserV.totalDeaths = int.Parse(ss[1]);
                LocalUserV.totalZombieKills = int.Parse(ss[2]);
                LocalUserV.totalZombieDeaths = int.Parse(ss[3]);
                print("stats set" + LocalUserV.totalKills);
            }
        });
    }
    ...
    private UserView GetUserInfo(int userid)
    {
    ...
    public SortedList<float, UserView> highscoresZombie = new SortedList<float, UserView>();
    public SortedList<float, UserView> highscores = new SortedList<float, UserView>();
    public void KillsTop(bool zombie)
    {
        newThread(delegate()
        {
            try
            {
                ...
                string res = Write(sendfunc);
                int min = LocalUserV.totalZombieKills;
                int id = -1;
                SortedList<float, UserView> scores = (zombie ? highscoresZombie : highscores);
                scores.Clear();
                bool found = false;
                for (int i = 0; i < 32; i++)
                {
    ...
    public void SendWallMsg(int userid, string text)
    {
        newThread(delegate()
        {
            string sendfunc = SendFunction(int.Parse(mid), ap_id, sid, secret,
    ...
    public Dictionary<int, UserView> friends = new Dictionary<int, UserView>();
    ...
    public XmlSerializer respxml = new XmlSerializer(typeof(VK.response), new Type[] { typeof(UserView), typeof(VK.message_info) });
    ...
    private void LoadAvatar(UserView user)
    {
        _TimerA.AddMethod(delegate()
        {
    ...
    public void SendMsg(int uid, string message)
    {
        newThread(delegate()
        {
            ...
            catch { print("error message not sended to "+ uid); }
        });
    }
    ...
    public void SendChatMsg(string message)
    {
        newThread(delegate()
        {
            ...
            catch (Exception e) { print("error message not sended to " + e); }
        });
    }
