...
public class MenuWindow : WindowBase {
	...
	void Wnd1(int id){
		...
		DisableSounds = GUI.Toggle(new Rect(333f, 293f, 116f, 15.96f),DisableSounds, new GUIContent(@"Disable Sounds",""));
		if (DisableSounds != oldDisableSounds ) {Action(MenuWindowEnum.DisableSounds);onButtonClick(); }
		onMouseOver = new Rect(333f, 293f, 116f, 15.96f).Contains(Event.current.mousePosition);
