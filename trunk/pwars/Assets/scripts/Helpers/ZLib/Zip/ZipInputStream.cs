...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	public class ZipInputStream : InflaterInputStream
	{
		...
		{
			publicReader = new ReadDataHandler(ReadingNotAvailable);
		}
		...
		public ZipEntry GetNextEntry()
		{
			...
			if (entry.IsCompressionMethodSupported()) {
				publicReader = new ReadDataHandler(InitialRead);
			} else {
				publicReader = new ReadDataHandler(ReadingNotSupported);
			}
		...
		int InitialRead(byte[] destination, int offset, int count)
		{
			...
			if ((csize > 0) || ((flags & (int)GeneralBitFlags.Descriptor) != 0)) {
				...
				publicReader = new ReadDataHandler(BodyRead);
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			return publicReader(buffer, offset, count);
		}
