...
public class AstarPath : MonoBehaviour {
	...
	private static int lastLogEntry = -1;
	...
	/*public static void StartPathPreThread (Path p, Seeker s) {
		ThreadDataHolder h = new ThreadDataHolder (p,s);
		Thread thread = new Thread(StartPathThread);
		thread.Priority = System.Threading.ThreadPriority.Highest;//Lowest;//BelowNormal;
		thread.Start (h);
	}
	...
	public class ThreadDataHolder {
		...
		public ThreadDataHolder (Path _p,Seeker _s) {
			p = _p;
			s = _s;
		}
	}
	
	public static void StartPathThread (System.Object h) {
		ThreadDataHolder h2= (ThreadDataHolder)h;
		Seeker s = h2.s;
		Path p = h2.p;
		...
		if (!p.error) {
			...
			//The error can turn up in the Init function
			if (!p.error && !p.foundEnd) {
				p.Calc (true);//Comment this if you want a bit higher framerate @Performance
				//while (!p.foundEnd && !p.error) {
	...
	public class Path {
		...
		public void Open (int i) {
			...
			if (node.script != this) {
				...
				node.extraCost =  Mathf.RoundToInt (node.basicCost*connection.angle*angleCost);
	...
	public void RecalcNeighbours (Node node) {
		...
		if (z != 0) {
			...
			if (x != 0) {
				if ((bottomLeftCut==2 || !dontCutCorners) && isNeighbours==IsNeighbour.Eight) {
					node.connections[5].endNode = GetNode(x-1,y,z-1);
					node.connections[5].enabled = GetNode(x-1,y,z-1).walkable && node.connections[5].angle < staticMaxAngle;
				}
			}
			
			if (x != grid.width-1) {
				if ((bottomRightCut==2 || !dontCutCorners) && isNeighbours==IsNeighbour.Eight) {
					node.connections[7].endNode = GetNode(x+1,y,z-1);
					node.connections[7].enabled = GetNode(x+1,y,z-1).walkable && node.connections[7].angle < staticMaxAngle;
				}
			}
		}
		...
		if (z != grid.depth-1) {
			...
			if (x != 0 && (topLeftCut==2 || !dontCutCorners) && isNeighbours==IsNeighbour.Eight) {
				node.connections[0].endNode = GetNode(x-1,y,z+1);
				node.connections[0].enabled = GetNode(x-1,y,z+1).walkable && node.connections[0].angle < staticMaxAngle;
			}
			
			if (x != grid.width-1 && (topRightCut==2 || !dontCutCorners) && isNeighbours==IsNeighbour.Eight) {
				node.connections[2].endNode = GetNode(x+1,y,z+1);
				node.connections[2].enabled = GetNode(x+1,y,z+1).walkable && node.connections[2].angle < staticMaxAngle;
			}
		}
