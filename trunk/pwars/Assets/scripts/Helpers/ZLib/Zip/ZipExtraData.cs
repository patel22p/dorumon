...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	/// The data is held publicly using a copy on write strategy.  This is more efficient but
	...
	sealed public class ZipExtraData : IDisposable
	{
		...
		public bool Find(int headerID)
		{
			...
			while ( (localTag != headerID) && (_index < _data.Length - 3) ) {
				localTag = ReadShortpublic();
				localLength = ReadShortpublic();
		...
		/// <summary>
		/// public form of <see cref="ReadShort"/> that reads data at any location.
		/// </summary>
		/// <returns>Returns the short value read.</returns>
		int ReadShortpublic()
		{
