        ...
        {                                                
            public void Start()
            {
                ...
                while (true) //Update()
                {
                    ...
                    //{
                    //    for (int i = icq._Users.Count - 1; i >= 0; i--)
                    //    {
                    //        User user = icq._Users[i];
                    //        if (DateTime.Now - user._DateTime > TimeSpan.FromDays(3))
                    //        {
                    //            icq.Remove(user);
                    //        }
                    //    }
        ...
        {            
            ...
            class HttpClient
            {
                ...
                private string OnMessage(string msg)
                {
                    Match m;
                    Trace2("OnMessage");
                    string ip = (((IPEndPoint)_Socket.RemoteEndPoint).Address).ToString(); //Regex.Match(@"\d+\.\d+.\d+",(((IPEndPoint)_Socket.RemoteEndPoint).Address).ToString()).Value;
